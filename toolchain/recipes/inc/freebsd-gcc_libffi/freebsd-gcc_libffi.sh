...
DEPENDENCIES="../make $arch-$freebsd_os-gcc"
DIR_DEPENDENCIES="../inc/freebsd-gcc_libffi"

recipe_start
export PATH="$OUT/make/bin:$OUT/$arch-$freebsd_os-gcc/usr/bin:$PATH"
gzip -d -c "$(recipe_download "https://github.com/libffi/libffi/releases/download/v$version/libffi-$version.tar.gz" "$sha512")" | tar xf -
cd "./libffi-$version"

./configure --prefix=/usr --build="$(sh ./config.guess)" --host="$arch-x-$freebsd_os" --disable-dependency-tracking CC= CPP= CXX=no CFLAGS="-ffile-prefix-map=$OUT=."
